            ...
            <div class="row">
                <div class="col-lg-8 mb-5 mb-lg-0">
                    <div class="blog_left_sidebar">
                        ...
                        $file = $_SERVER['DOCUMENT_ROOT'] . '/data/articles.json';
                        if (file_exists($file)) {
                            $newsData = json_decode(file_get_contents($file), true);
                        
                            if (!empty($newsData)) {
                                foreach ($newsData as $news) {
                                    // Generate the link to the specific article page
                                    $articleLink = '/articles/' . htmlspecialchars($news['title']) ."-".htmlspecialchars($news['id']).'.php';

                                    echo '
                                    <article class="blog_item">
                                        <div class="blog_item_img">
                                            <img class="card-img rounded-0" src="' . htmlspecialchars($news['image']) . '" alt="">
                                            <a href="' . $articleLink . '" class="blog_item_date">
                                                <h3>' . date('d', strtotime($news['date'])) . '</h3>
                                                <p>' . date('M', strtotime($news['date'])) . '</p>
                                            </a>
                                        </div>
                                        <div class="blog_details">
                                            <a class="d-inline-block" href="' . $articleLink . '">
                                                <h2>' . htmlspecialchars($news['title']) . '</h2>
                                            </a>
                                            <p>' . htmlspecialchars(mb_strimwidth($news['content'], 0, 150, '...')) . '</p>
                                        </div>
                                    </article>';
                                }
                            } else {
